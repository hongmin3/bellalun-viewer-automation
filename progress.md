...
- 현재 목표: `NEXT_WORK.md` P0 순서대로 계속한다. `KeyError` 픽스는 라이브 검증까지
  끝났고(아래), 다음은 새로 드러난 "목록 전 행 열거 완주" 읽기 문제 조사 또는
  `cold_start` 주 모니터 밖 확인(P0 #2)이다.
...
- **`KeyError` 픽스 라이브 검증 완료(같은 세션).** `run-wf14`를 다시 돌렸다
  ...
  **대신 새로 드러난 문제**: "목록 전 행 열거 완주" 서브체크가 계속 FAIL 한다 —
  대부분의 목록 페이지(`patient.physician`/`display.overlay`/`display.lut`/
  `procedure.procedure`/`dicom.print_overlay`/`dicom.tag_mapping`/`qc.scheduler` 등)에서
  행 문구를 못 읽거나 열거한 행 수가 DB 원천 행 수와 다르다. **Import 전(기준값 수집)과
  Import 후 양쪽에서 완전히 동일한 패턴** — Export/Import와 무관하게 이 화면의 행
  텍스트 읽기 메커니즘(`core/setting_lists.py::_screen`) 자체가 의심된다. `stop=False`라
  TC를 막지는 않고, 실제 값 대조는 "불완전한 열거는 근거로 안 쓴다"는 설계 덕에
  오염되지 않아 지금까지 방치돼 있었던 것으로 보인다. **원인 미확인 — 라이브 조사
  필요.**

- 남은 작업(다음 세션 시작점):
  1. **"목록 전 행 열거 완주" 읽기 문제 원인 조사.** `core/setting_lists.py::_screen`이
     왜 다수 페이지에서 행 문구를 못 읽는지, DB 원천 행 수 쿼리가 화면과 다른 것을
     세는 건 아닌지 라이브로 확인한다. 추측하지 말고 실측한다.
