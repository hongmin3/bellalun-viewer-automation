...
- 사용자가 "목록 전 행 열거 완주" 수정 방향으로 **옵션 1(OCR 전면 재설계)** 을
  승인해 착수 전 정찰로 `LIST_PAGES` 13개 페이지를 전부 라이브로 OCR 확인했다
  (`NEXT_WORK.md` 5절 ⑦ "13개 페이지 전수 라이브 확인 결과"). **OCR 전환만으로는
  4곳만 해결된다** — 나머지 3곳은 스크롤 완주 문제로 보이고(재확인 필요), 5곳은
  OCR 과 무관한 페이지별 설계 문제(DB 카운트가 화면과 다른 것을 셈 3곳, 교차 오염
  2곳, 화면 요소 자체가 틀림 1곳 — 합 5곳 맞음, 표 참고)다. **구현 범위(①만 먼저
  끝낼지 전부 다룰지)를 사용자에게 다시 확인해야 한다 — 아직 코드는 건드리지 않았다.**

- 남은 작업(다음 세션 시작점):
  1. 위 구현 범위를 사용자와 확인한 뒤 `core/setting_lists.py` 수정 착수.
