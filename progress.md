...
- 다음 세션(또는 화면이 충분히 오래 풀려 있을 때)에서 시작할 작업: 포그라운드 창 타이틀(+`EnumWindows`로 잠금 화면 부재 확인)로 잠금 해제와 **유지 시간**을 확인한 뒤, 위 "남은 작업" 1번(`run-wf07`/`run-xipl-04` 재검증)부터 순서대로 진행. 재잠금 주기가 짧다면 한 TC를 다 못 끝낼 수 있으니, 시도 전에 사용자에게 화면 유지가 필요하다고 알리는 것을 고려한다.
