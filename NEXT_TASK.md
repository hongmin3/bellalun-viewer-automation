...
### Examined 목록을 띄우는 데서 막혔다 (다음 세션 출발점)

버튼 기능을 확정하려면 목록에 검사가 있어야 하는데, 아직 띄우지 못했다. 실측:

- Examined 창의 기간 버튼은 Patient 화면과 **같은 ID를 쓴다**: Month = `1108`
  (실측 rect `(892,163,972,200)`), 검색 돋보기 = `2180` (rect `(1314,133,1358,177)`).
- 그런데 `1108`을 클릭한 뒤 `2180`으로 검색해도 **기간 표시가
  `2026/08/18 ~ 2026/08/18`에서 바뀌지 않고 목록도 비어 있었다.** 클릭이 적용되지
  않은 것으로 보인다(이번 세션에서 반복 확인된 "조작 후 확인 없음" 패턴일 가능성 —
  누른 뒤 기간 라벨이 바뀌었는지 확인하며 재시도해야 한다).
- DB에는 완료 검사가 있다(`STUDY` Key 51/52, `StudyStatus=3`, 날짜 2026/08/18).
  그래서 "검사가 없어서 안 보이는 것"은 아니고, **목록 필터 조건을 더 봐야 한다.**
  툴바 왼쪽의 `All` 드롭다운(모달리티/상태 필터로 추정)도 함께 확인할 것.
- 참고: `StudyStatus` 값은 실측으로 1=진행 중, 3=완료, 4=보류로 나타났다.

정리하면 다음 세션의 순서는 (1) Examined 목록 띄우기 → (2) 2184/2197이 각각
Send/Export인지 확인 → (3) WF05 드라이버 → (4) WF04 Export 검증이다.
