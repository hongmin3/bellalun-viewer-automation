...
1. ~~`run-wf14`를 다시 돌려 `KeyError: 'changed'` 픽스를 라이브로 검증한다~~ —
   **2026-08-31 완료.** 예외 없이 Step 7(c)까지 PASS로 판정됐다(2-D절). TC 전체 FAIL은
   이미 알려진 UPS 제품 결함(3-A/B.22)이라 정상이다. **대신 새 항목**: "목록 전 행 열거
   완주" 서브체크가 계속 FAIL 한다(2-D절 "라이브 재검증 결과") — `core/setting_lists.py`
   의 행 텍스트 읽기(`_screen`)가 다수 목록 페이지에서 문구를 못 읽거나 DB 원천 행 수와
   다르게 센다. Export/Import 전후 동일하게 나타나 Import 결함이 아니라 **읽기
   메커니즘 자체의 문제로 보이지만 확인 안 됨** — `core/setting_lists.py::_screen`을
   목록 페이지별로 스크린샷·컨트롤 열거와 함께 라이브로 조사해 원인을 찾아라. 추측하지
   말고 실측해라. 원래 3-C 증상(`my_settings` 진입 실패)은 연속 4회 모두 재현 안 됨 —
   열어는 두되 급하지 않다.
