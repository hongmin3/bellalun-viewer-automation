...
**결론(2026-08-31 갱신, `../지식/` 사양서 대조 + DB 직접 조회 후)**: ①(4곳)과
③-a(`display.lut`)는 완료됐다. ②(스크롤 미완주, 4곳)는 추정이 아니라 사양서로
원인이 확정됐다 — 자동화의 스크롤 완주 로직을 고치는 문제다. ③(개념 불일치,
`display.overlay` 1곳 남음)은 새 검증 방식(항목별 추가/미추가 상태 비교)이 필요하다.
