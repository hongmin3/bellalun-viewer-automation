...
2. **"목록 전 행 열거 완주" 서브체크가 실패하는 근본 원인을 라이브로 확인했다(2-D절
   "목록 열거 읽기 결함의 근본 원인").** 원인 두 가지가 섞여 있다 — (a) 행이
   owner-draw 라 자식 창이 없어 `row_signature()`(WM_GETTEXT 기반)가 항상 빈 문자열을
   돌려준다, (b) `patient.physician` 은 `visible_rows()`가 실제 Physician 목록이 아닌
   **다른 콤보/목록**(MWL 역할 매핑 옵션 3개)을 잘못 집고 있다. 다음에 할 일: OCR
   기반으로 `row_signature()`를 다시 설계할지, 페이지 매핑부터 바로잡을지 사용자와
   합의한 뒤 진행한다(급하지 않다 — `stop=False`라 다른 판정을 오염시키지 않는다).
