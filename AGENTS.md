...
1. Read `README.md`, `NEXT_TASK.md`, the user-attached checklist, and the relevant manuals/specifications before changing automation behavior. Also read the durable guidance in `..\지식\` — especially `[자동화 운영 지침] Bellalun Viewer auto 저장소 구현 규칙.md` (implementation rules for this repo: state-based waiting, log-timestamp filtering, PASS/FAIL evidence policy, GPU-less handling, fixture freshness, session-separation) and `[QA 작성 규칙] Bellalun Viewer TC 설계·자동화·자체검토 가이드.md` (TC design methodology). `HANDOFF.md`, when present, is a one-off per-session handoff only — durable rules live in the `지식` folder, not there.
