...
> 이 문서는 **저장소 작업 규칙**이다 — 기준 문서가 무엇인지, 어떤 순서로 일하는지, 긴 실행 전에 무엇을 검사하는지, 어떻게 커밋하는지.
> **지금 상태와 다음 할 일**은 `NEXT_WORK.md`, **과거에 실측으로 확정한 사실**은 `NEXT_TASK.md`, **코드를 어떻게 짜야 하는가(영구 구현 규칙과 사고 이력)** 는 `..\지식\[자동화 운영 지침] ...md` 에 있다. 절차가 겹치면 **이 문서가 우선**이고, 그 이유와 사고 경위는 운영 지침이 자세하다.
> **읽는 순서 — `README.md` 최상단 "온보딩 요약" → 이 문서 → `NEXT_WORK.md` → `[자동화 운영 지침]`(상단 "증상 → 원인 → 조치" 색인부터) → 필요할 때 `NEXT_TASK.md` 검색.**
...
1. **자동화 동작을 바꾸기 전에 먼저 읽는다** — `README.md`, **`NEXT_WORK.md`**,
   `NEXT_TASK.md`, 사용자가 첨부한 체크리스트, 그리고 관련 매뉴얼·사양서.
   (`NEXT_WORK.md` 가 **지금 상태와 다음 할 일**이고 `NEXT_TASK.md` 는 **누적 기록**이다.
   맨 위 "문서 지도" 참고.)
   `..\지식\` 폴더의 영구 지침도 함께 본다.
...
10. **절대 커밋하지 않는다** — `config.json`, 자격증명, 토큰, 환자·런타임 데이터,
    ...
    내용. **`Archive/` 는 예외로 커밋한다** — 내려 둔 기록이 저장소 안에 남아 있어야
    "지운 것이 아니라 옮긴 것"이 성립한다.

11. 공개된 이력을 다시 쓰거나 force-push 하지 않는다. 커밋이나 push가 실패하면
    완료했다고 주장하지 말고 **정확한 원인을 보고한다.**

## 문서 수명 정책 — 낡은 기록은 **지우지 말고 내린다**

이 저장소는 "실패를 규칙으로 승격한다"는 방침 때문에 문서가 계속 자란다. 그 자체는
옳지만, 새 세션이 상태를 파악하려고 `README.md` + `AGENTS.md` + `NEXT_WORK.md` +
...
**그래서 끝난 일은 `Archive/` 로 내린다. 지우는 것이 아니다.** 원문 그대로 옮기고
원래 자리에 어디로 갔는지 한 줄을 남기므로, 검색하면 그대로 나오고 Git 이력에도
...
| 문서 | 정책 | 이유 |
|---|---|---|
| `HANDOFF.md` | **보존 7일. 지나면 삭제**(커밋된 경우에만) | 일회성 인수인계다. 영구 규칙은 여기 두지 않는다(운영 지침 15절). 커밋 전이면 Git 이력에도 없으므로 지우지 않고 경고만 한다 |
| `NEXT_TASK.md` | 해결 표시가 붙은 절 + 보존 기간(60일) 지난 날짜 절 → `Archive/NEXT_TASK_완료기록.md` | 누적 기록이지만 **끝난 일은 다음 작업에 필요 없다.** 실측 컨트롤 ID·확정한 제품 동작은 해결 표시를 붙이지 않으므로 남는다 |
| `NEXT_WORK.md` | 이관하지 않는다. 줄 수 예산만 점검(400줄) | 현재 상태 문서다. 회차가 끝나면 누적 기록을 `NEXT_TASK.md` 로 손으로 옮긴다 |
| `AGENTS.md` · `README.md` · 지식 지침 2종 | **이관 대상 아님.** 줄 수 예산만 점검 | 영구 규칙과 포트폴리오 요약이다. 여기서 뭔가를 내려야 할 만큼 커졌다면 그건 사람이 판단할 일이다 |

**해결 표시**는 절 제목의 `[완료]` `(완료)` `(해소됨)` `(해결됨)` 과 취소선(`~~`)이다.
끝난 항목은 **지우지 말고 제목에 이 표시를 붙인다** — 그러면 다음 실행에서 자동으로
내려간다. 반대로 **절 안에 `<!-- keep -->` 을 넣으면 절대 옮기지 않는다.**

**지키는 것 세 가지.**

1. **정보를 잃지 않는다.** 이관은 이동이지 삭제가 아니다. 유일한 삭제는 `HANDOFF.md`
   이고, 그것도 커밋돼 Git 이력에 남은 뒤에만 지운다.
2. **실측 지식은 문서에서 내리기 전에 코드로 승격한다.** 컨트롤 ID·임계값·파일명
   규칙은 `core/` 의 상수나 `automation_scope.json` / `traceability.json` 에 있어야
   하고, 문서는 그 경위를 적는 자리다. 승격하지 않은 실측값이 든 절에는
   `<!-- keep -->` 을 붙인다.
3. **줄 수 예산은 상한이 아니라 신호다.** 넘었다고 자동으로 지우지 않는다. 무엇을
   내릴지는 사람이 정하고, 도구는 "넘었다"와 "무엇이 낡아 가는가"만 알려 준다.
